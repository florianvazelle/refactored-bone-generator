...
impl proto::bone_generator_server::BoneGenerator for MyBoneGenerator {
    async fn execute(&self, request: Request<proto::Members>) -> Result<Response<proto::Members>, Status> {
        println!("Got a request from {:?}", request.remote_addr());

        let members = &request.get_ref().members;

        let parsed_request = members.iter()
        ...
        let parsed_reply = pca::compute_bones_generation(&parsed_request)
            ...
            .map(|(name, points_cloud)| {
                let vertices: Vec<proto::Vertex> = points_cloud
                            .iter()
                            .map(|point| proto::Vertex { x: point.x, y: point.y, z: point.z })
                            .collect();
                proto::Member { name: name.to_string(), vertices }
            }).collect();
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    Server::builder()
        .add_service(proto::bone_generator_server::BoneGeneratorServer::new(bone_generator))
        .serve(addr)
        .await?;
