...
fn compute_barycenter(data: &Vec<Vector3<f64>>) -> Vector3<f64> {
...
fn transform_space(axis: &Vector3<f64>, data: &Vec<Vector3<f64>>) -> Vec<Vector3<f64>> {
    data.iter().map(|value| value - axis).collect()
}

fn compute_covmatrix(data: &Vec<Vector3<f64>>) -> Matrix3<f64> {
...
pub fn compute_bones_generation(members: &Vec<(String, Vec<Vector3<f64>>)>) -> Vec<(String, Vec<Vector3<f64>>)> {
    let mut principal_components: Vec<(String, Vec<Vector3<f64>>)> = Vec::with_capacity(members.len());
    ...
    for (key, points_cloud) in members.iter() {
        ...
        let proj: Vec<Vector3<f64>> = center_points_cloud.iter().map(|point| (point.dot(&eigenvector) / (norm * norm)) * eigenvector).collect();
